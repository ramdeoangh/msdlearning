...
<?php else : ?>
	<!-- <div class="w-100">
     ...
     <div class="w-100 iframe">
        <?php
        $attachment = $lesson_details['attachment'];
        if (strpos($attachment, '<iframe') !== false || strpos($attachment, '&lt;iframe') !== false) {
            echo html_entity_decode($attachment, ENT_QUOTES, 'UTF-8');
        } else {
            $safe_url = htmlspecialchars($attachment, ENT_QUOTES, 'UTF-8');
            ?>
            <iframe class="embed-responsive-item" width="100%" height="550px" src="<?php echo $safe_url; ?>" allowfullscreen></iframe>
        <?php } ?>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var watermarkSpeed = '<?php echo get_frontend_settings('water_mark_speed'); ?>'; 
        ...
        if (watermarkStatus === 'active') {
            ...
            var watermark;
            var obfuscatedClass = 'wm_' + Math.random().toString(36).substr(2, 9);
            ...
            function createWatermark() {
                ...
                if (!watermark || !document.body.contains(watermark)) {
                    ...
                    // Apply styles
					watermark.style.color = watermarkColor || '#fff';
                    watermark.style.position = 'absolute';
                    watermark.style.visibility = 'hidden';
                    watermark.style.top = '20px';
                    watermark.style.left = '15px';
                    watermark.style.opacity = watermarkOpacity || '0.6'; // Use dynamic opacity
                    watermark.style.pointerEvents = 'none';
                    watermark.style.width = '150px';
                    watermark.style.transition = `transform 1.3s ease`; // Keep this for smooth movement
            ...
            function monitorWatermark() {
                if (watermark.style.display === 'none' || watermark.style.opacity === '0') {
                    ...
                    watermark.style.opacity = watermarkOpacity || '0.6';
                }
                if (!watermark || !document.body.contains(watermark)) {
                    createWatermark();
                    showWatermark();
            ...
            setInterval(animateWatermark, parseInt(watermarkSpeed) || 2000); // Use dynamic speed
